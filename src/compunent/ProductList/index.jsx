...
const ProductList = () => {
  const { products,ProductListDealete,showProductDetails,filterProducts  } = useContext(ProductItemContext);
...
//     const filterdata = products.filter((e)=>e.productName.toLowerCase().includes(searchText.toLowerCase()))
// }

  const showProductDetailsfg = (product) => {
    // Assuming you want to navigate to a product details page
    navigate('/product-datials'); // Pass product data here// Update the route as per your requirement
    showProductDetails(product)
  };

  
  const showProductEditrfg = (product) => {
    // Assuming you want to navigate to a product details page
    navigate('/product-edit', { state: { product } }); // Pass product data here// Update the route as per your requirement
    showProductDetails(product)
  };
  ...
  console.log(products, "in listt");

  return (
<div className="p-6 bg-gray-100">
  {/* Display the Hello message */}
  <h1 className="text-2xl font-bold mb-6 text-center">Hello, welcome to our product list!</h1>
      ...
      <tbody>
        {  products.length > 0 ? (
          filteredProducts.map((product, index) => (
            ...
            <td class="p-4 text-sm">$ {product.price}</td>
            <td class="p-4 text-sm">{product.stock}</td>
            <td class="p-4 text-sm">{product.category}</td>
            <td class="p-4 text-sm"> {product.brand}</td>
            <td class="p-4 text-sm">
              <div class="flex items-center">
                <p class="w-[50%] h-[60%] rounded-lg bg-yellow-100 text-yellow-800 flex items-center justify-center">{product.rating}</p>
            ...
            <td class="p-4 flex items-center mt-6">
              <button class="mr-4 flex gap-2 items-center text-blue-500 hover:text-blue-700" title="Edit">
                <i onClick={() => showProductDetailsfg(product)} class="ri-eye-line text-lg"></i>
              ...
              <button onClick={() => showProductEditrfg(product)}>
                edit
              </button>
              <button onClick={()=>ProductListDealete(product.productName)} class="mr-4 flex items-center text-red-500 hover:text-red-700" title="Delete"> 
          ...
          <tr>
            <td colSpan="6" className="p-3 text-center text-gray-600">No products available</td>
          </tr>
        )}
      </tbody>
    </table>
